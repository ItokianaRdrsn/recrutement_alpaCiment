...
{
    /**
     * Run the migrations for offre, profil_offre, mission, profil_formation matching exact gestion_recrutement.sql
     */
    public function up(): void
    {
        if (! Schema::hasTable('offre')) {
            Schema::create('offre', function (Blueprint $table) {
                ...
                $table->foreignId('id_direction')->constrained('direction', 'id_direction')->restrictOnDelete();
                $table->text('description')->nullable();
                $table->string('lieu', 200)->nullable();
                $table->foreignId('id_type_contrat')->nullable()->constrained('type_contrat', 'id_type_contrat')->restrictOnDelete();
                $table->date('date_publication')->nullable();
                $table->date('date_limite')->nullable();
                $table->foreignId('id_statut_offre')->constrained('statut_offre', 'id_statut_offre')->restrictOnDelete();
                $table->timestampsTz();
        ...
        if (! Schema::hasTable('profil_offre')) {
            Schema::create('profil_offre', function (Blueprint $table) {
                $table->id('id_profil_offre');
                $table->foreignId('id_offre')->unique()->constrained('offre', 'id_offre')->cascadeOnDelete();
                $table->text('description')->nullable();
