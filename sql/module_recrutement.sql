...
CREATE TABLE candidat (
    ...
    email VARCHAR(200) NOT NULL UNIQUE,   -- unique : un candidat qui repostule est retrouvé par email, pas dupliqué
...
CREATE INDEX idx_competence_nom_trgm
    ON competence USING GIN (nom_competence gin_trgm_ops);
...
CREATE TABLE competence_alias (
    ...
    texte_brut VARCHAR(150) NOT NULL UNIQUE,   -- texte normalise (minuscules, sans accents)
...
CREATE TABLE candidature (
    ...
    canal_depot VARCHAR(20) NOT NULL DEFAULT 'site_externe'
        CHECK (canal_depot IN ('site_externe', 'rh_manuel')),
    ...
    CONSTRAINT chk_candidature_canal CHECK (
        canal_depot = 'site_externe' OR id_utilisateur_depot IS NOT NULL
    )
);
...
CREATE TABLE document (
    ...
    mode_acquisition VARCHAR(20) NOT NULL DEFAULT 'fichier'
        CHECK (mode_acquisition IN ('fichier', 'photo_ocr')),
    ...
    recherche_texte tsvector
        GENERATED ALWAYS AS (to_tsvector('french', coalesce(contenu_texte_extrait, ''))) STORED,
...
CREATE TABLE candidat_competence (
    ...
    source VARCHAR(20) NOT NULL DEFAULT 'manuel'
        CHECK (source IN ('manuel', 'cv_ocr')),
    score_confiance NUMERIC(4,3),          -- 0 a 1, pertinent si source = 'cv_ocr'
    id_document BIGINT
        REFERENCES document(id_document)
        ON DELETE SET NULL,                -- CV source si extrait automatiquement
    ...
    CONSTRAINT chk_candidat_competence_validation CHECK (
        valide = FALSE OR (valide_par IS NOT NULL AND date_validation IS NOT NULL)
    )
);
...
CREATE TABLE experience_professionnelle (
    ...
    date_debut DATE,
    date_fin DATE,                         -- NULL si poste actuel
...
CREATE TABLE formation (
    ...
    niveau VARCHAR(50),                    -- ex: Bac+3, Bac+5, Doctorat...
    date_obtention DATE,
...
SELECT
    date_trunc('month', c.date_candidature)::date AS mois,
    COUNT(*) AS total_candidatures,
    COUNT(*) FILTER (WHERE sc.libelle = 'Retenue') AS candidatures_retenues,
    ROUND(
        COUNT(*) FILTER (WHERE sc.libelle = 'Retenue')::numeric
        / NULLIF(COUNT(*), 0) * 100,
    2) AS taux_transformation_pct
FROM candidature c
JOIN statut_candidature sc ON sc.id_statut_candidature = c.id_statut_candidature
GROUP BY 1
ORDER BY 1;
...
CREATE VIEW vue_stats_delai_traitement AS
WITH premiere_reception AS (
    SELECT hs.id_candidature, MIN(hs.date_changement) AS date_reception
...
statut_final AS (
    SELECT hs.id_candidature, MIN(hs.date_changement) AS date_decision
    FROM historique_statut hs
    JOIN statut_candidature sc ON sc.id_statut_candidature = hs.id_statut_candidature
...
SELECT
    date_trunc('month', pr.date_reception)::date AS mois,
    ROUND(AVG(
        EXTRACT(EPOCH FROM (sf.date_decision - pr.date_reception)) / 86400
